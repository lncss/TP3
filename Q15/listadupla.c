...
void mostrar (){
   Celula* i;
   for(i = primeiro -> prox; i != NULL; i = i -> prox) movie_print(&(i -> elemento));
}
...
void sort() { 
    ...
    for(Celula* i = primeiro -> prox; i != NULL; i = i -> prox, j++) array[j] = i;

    quicksort(array, 0, tam - 1); 
}

void quicksort(Celula** array, int esq, int dir) {

    int i = esq, j = dir;

    Movie pivo = array[(esq + dir) / 2] -> elemento;

    while(i <= j) {

        if(strcmp(array[i] -> elemento.situation, pivo.situation) == 0) {

            while(strcmp(array[i] -> elemento.name, pivo.name) < 0) i++;
        }
        else {

            while(strcmp(array[i] -> elemento.situation, pivo.situation) < 0) i++;
        }

        if(strcmp(array[j] -> elemento.situation, pivo.situation) == 0) {

            while(strcmp(array[j] -> elemento.name, pivo.name) > 0) j--;
        }
        else {

            while(strcmp(array[j] -> elemento.situation, pivo.situation) > 0) j--;
        }

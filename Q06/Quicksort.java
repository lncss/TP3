...
class Lista {
    ...
    private void quicksort(int esq, int dir) {
        ...
        while (i <= j) {
            while(array[j].getSituation().compareTo(pivo.getSituation()) < array[j].getSituation().compareTo(pivo.getSituation())) i++;
            while(array[j].getSituation().compareTo(pivo.getSituation()) > array[j].getSituation().compareTo(pivo.getSituation())) j--;
            if (i <= j) {
                swap(i, j);
                i++;
                j--;
    ...
    public void insertsort() { // organiza de novo a lista ordenando por nome para desempatar 
        for (int i = 1; i < n; i++) {
            Filme tmp = array[i];
            int j = i - 1;
...
public class Quicksort{
    public static void main(String[] args)throws Exception{
        ...
        FileWriter arq = new FileWriter("747560_quicksort.txt");

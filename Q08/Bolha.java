...
class Lista {
    ...
    public void swap(int i, int j) {
      ...
      public void sort() {
		for (int i = (n - 1); i > 0; i--) {
			for (int j = 0; j < i; j++) {
				if (array[j].getDuration().compareTo(array[j+1].getDuration()) > array[j+1].getDuration().compareTo(array[j].getDuration())) {
                    swap(j, j+1);
                    comp++;
				}
			}
...
public class Bolha{
    public static void main(String[] args)throws Exception{
        ...
        FileWriter arq = new FileWriter("747560_bolha.txt");

...
int comp = 0;       // Calcular o numero de comparacoes 
...
void swap(Movie array[], int i, int j) {
   Movie tmp = array[i];
   array[i] = array[j];
   array[j] = tmp;
}

void selecao(Movie array[], int i, int n){
    int menor = i;

    for(int j = i+1; j < n; j++){
        if(strcmp(array[j].original_title, array[menor].original_title) < 0){
            menor = j;
            comp++;
        }
    }
...
int main() {
    ...
    //abrir o arquivo 
    pont_arq = fopen("747560_selecao.txt", "w");
    ...
    fprintf(pont_arq, "747560\t Comparacoes %i\t Tempo %lf\t",comp,((double)(fim - inicio)/CLOCKS_PER_SEC));
